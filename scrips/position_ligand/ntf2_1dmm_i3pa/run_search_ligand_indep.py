...
'''
python /mnt/e/GitHub_Design/Metalprot/scrips/position_ligand/ntf2_1dmm_i3pa/run_search_ligand_indep.py 0 4
'''
...
ro1 = ['C10', 'C9']
rest1 = ['H7', 'C11', 'O1', 'O3', 'O2', 'FE1']
ro2 = ['C9', 'C8']
rest2 = ['H7', 'H6', 'H5', 'C10', 'C11', 'O1', 'O3', 'O2', 'FE1']
...
def run_all(file, workdir, path_to_database, ideal_ala_coords, lig_path):
    ...
    for lig_id in lig_vdm_dict.keys():
        ...
        for v in values:
            if v[1][0] == 'indole' and v[1][-2] == True:
    ...
    for lig_id in lig_vdm_dict.keys():
        ...
        for v in values:
            ...
            if (cg, aa, resnum, vdm_id) in unique_vdms.keys():
                continue
            else:
                unique_vdms[(cg, aa, resnum, vdm_id)] = (vdm_ag, v)

    for (cg, aa, resnum, vdm_id) in unique_vdms.keys():
        prefix = cg  + '_' + aa + '_' + str(resnum) + '_' + str(vdm_id)                 
        pr.writePDB(outdir_uni + prefix  + '.pdb.gz', unique_vdms[(cg, aa, resnum, vdm_id)][0])
...
def main():
    ...
    if on_wynton:
        ...
        lig_path = '/mnt/e/DesignData/ligands/LigandBB/_lig_fe/i3pa_maestro.pdb'
