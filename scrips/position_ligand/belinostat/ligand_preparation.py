...
workdir = '/mnt/e/DesignData/Metalloenzyme/belinostat/ligs/'

moe_indir = workdir + 'meo_50g_amber14eht_md/'
meo_outdir = workdir + 'meo_50g_amber14eht_md_out2/'
...
for i in range(len(moe_ligs)):
    moelig = moe_ligs[i]
    mob_coord = [moelig.select('name ' + x)[0].getCoords() for x in ['N2', 'C12', 'O3', 'C11']]
    target_coord = [lig.select('name ' + x)[0].getCoords() for x in ['N2', 'C12', 'O3', 'C11']]
