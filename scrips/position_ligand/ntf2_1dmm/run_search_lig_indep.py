...
def run_search(target, ligs, path_to_database, ideal_ala_coords, para):
    '''
    cg = 'phenol'
    resnum = 102
    '''
...
def run_all(file, workdir, path_to_database, ideal_ala_coords, lig_path, para):
    ...
    outdir = workdir + '1dmm_tts_results/output_' + file + '_' + time_tag + '/'
    ...
    lig_vdm_dict = run_search(target, ligs, path_to_database, ideal_ala_coords, para)
    ...
    for lig_id in lig_vdm_dict.keys():
        ...
        if para.find_unknow and not exist_required_cg_contact:
            continue
...
def main():
    ...
    print(path)
    print(para.predefined_win_filters)
