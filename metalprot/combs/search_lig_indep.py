...
def get_vdm_labels_coords_4old_vdm_db(dfa, correspond_resname, correspond_names):
    '''
    Example:
...
def search_lig_at_cg_aa_resnum(target, chidres, chidres2ind, pos, abple, ligs, vdm_cg_aa_atommap_dict, cg, df_vdm, ideal_ala_coords, rmsd = 0.7, filter_hb = False, filter_cc = False):
    ...
    for cg_id in vdm_cg_aa_atommap_dict.keys():
        if vdm_cg_aa_atommap_dict[cg_id]['cg'] != cg:
            continue
        #print(input_dict[cg_id]['lgd_sel'])
        #print(_ligs)
        ligand_coords = get_ligand_coords(_ligs, vdm_cg_aa_atommap_dict[cg_id]['lgd_sel'])

        labels, vdm_coords = get_vdm_labels_coords_4old_vdm_db(df_vdm, vdm_cg_aa_atommap_dict[cg_id]['represent_name'], vdm_cg_aa_atommap_dict[cg_id]['correspond_resname'], vdm_cg_aa_atommap_dict[cg_id]['correspond_names'])
...
def clash_filter_protein_single(target, chidres, chidres2ind, vdm_ag, dist_cut = 2.7):
    '''
    # clashing: any atom close to the bb let's say dist <= dist_cut
