...
for subfolder in subfolders_with_paths:
    paths = search_struct.extract_centroid_pdb_in_clu(subfolder + '/', file_path = '_summary.txt', score_cut = 0, clu_num_cut = 10)
...
for subfolder in subfolders_with_paths2:
    paths = search_struct.extract_centroid_pdb_in_clu(subfolder + '/', file_path = '/_summary.txt', score_cut = 1, clu_num_cut = 50)
...
for query in querys:
    for query_2nd in query_2nds:
        ...
        query_win, query_2nd_win, extracts = search_struct.build_core(outpath, target, query, query_2nd, rmsd_cut, rmsd_cut_2nd, distance_cut)
        if query_win == 0:
            break
