...
def get_all_cbs_depre(pdb, id_array_dist, num = 3):
    ...
    if (26, 27) in un_dist_dict or (27, 35) in un_dist_dict or (26, 35) in un_dist_dict:
        print('right')
    return all_cbs
...
def get_all_cbs(pdb, id_array_dist, nums = [3, 4]):
    ...
    print('paths_len: {}'.format(len(paths)))
    for path in paths:
...
def extract_aa_comb(pdb, id_array_dist, outdir):
    # all_cbs = get_all_cbs_depre(pdb, id_array_dist, 3)
    # all_cbs.extend(get_all_cbs_depre(pdb, id_array_dist, 4))
...
outdir = workdir + 'output_1a88_3/'
if not os.path.exists(outdir):
    os.mkdir(outdir)

pdb = pr.parsePDB(workdir + '1a88.pdb')
